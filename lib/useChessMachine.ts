    ...
    const checkForCheckCondition = (attempt = 1) => {
      ...
      if (snapshot.context?.board && snapshot.context?.currentPlayer) {
        import('./chessMachine').then(({ isKingInCheck }) => {
          ...
          const whiteInCheck = isKingInCheck(snapshot.context.board, 'white', snapshot.context.enPassantTarget, snapshot.context.castlingRights);
          const blackInCheck = isKingInCheck(snapshot.context.board, 'black', snapshot.context.enPassantTarget, snapshot.context.castlingRights);
